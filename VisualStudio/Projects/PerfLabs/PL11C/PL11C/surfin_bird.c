...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)
{
	char *return_ptr = NULL; //points to where word is
	int sentence_length = 0; //lenth of sentence
	int word_length = 0; //length of word
	int i = 0; //first iterator
	int j = 0; //second iterator
	
	if (!sentence_ptr) //return ERROR_NULL_SENTENCE_POINTER if sentence pointer is null
	{
		return_ptr = 0;
		*errorCode_ptr = ERROR_NULL_SENTENCE_POINTER;
	}
	else if (!searchWord_ptr) //return ERROR_NULL_SEARCH_POINTER if search pointer is null
	{
		return_ptr = 0;
		*errorCode_ptr = ERROR_NULL_SEARCH_POINTER;
	}
	else if (!errorCode_ptr) //return NULL if error code pointer is null
	{
		return_ptr = 0;
	}
	else //MEAT
	{
		sentence_length = strlen(sentence_ptr);
		word_length = strlen(searchWord_ptr);

		if (word_length > sentence_length) //if word is longer than sentence, by def it can't be found, so return error
		{
			return_ptr = 0;
			*errorCode_ptr = ERROR_SEARCH_NOT_FOUND;
		}
		else //if word is shorter than sentence then it COULD be contained in the sentence
		{
			*errorCode_ptr = ERROR_SEARCH_NOT_FOUND; //defaults error to SEARCH NOT FOUND
			for (i = 0; i <= (sentence_length); i++) //first iterator loop: search length of sentence (could iterate through i <= (sentence length - word length) but let's not overcomplicate things
			{
				if (*(sentence_ptr + i) == *searchWord_ptr) //if the value of sentence_ptr[i] equals searchWord_ptr[i]
				{
					if (word_length == 1) //if word is only 1 character and it is found already then you can be done
					{
						return_ptr = (sentence_ptr + i);
						*errorCode_ptr = ERROR_CODE_SUCCESS;
						return return_ptr;
					}
					else //if word isn't just one chararcter then look for the rest
					{
						for (j = 1; j < word_length; j++) //second iterator loop: check to see if rest of word is there, you start at j = 1 because you have already found the first letter
						{
							if (*(sentence_ptr + i + j) != *(searchWord_ptr + j)) //if next value in sequence doesn't match next value in searched word then get outta there
							{
								break;
							}
							else if (j == (word_length - 1)) //if you have searched the rest of the sentence and have found the word in order (aka if the word is complete)
							{
								return_ptr = (sentence_ptr + i); //mark the beginning of the word at the correct position
								*errorCode_ptr = ERROR_CODE_SUCCESS;
								return return_ptr;
							}
						}
