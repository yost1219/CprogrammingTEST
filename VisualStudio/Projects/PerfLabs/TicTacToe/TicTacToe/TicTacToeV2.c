...
int print_the_grid(char * ticTacToeGrid)
{
int gridSize = strlen( * ticTacToeGrid);
//pass the grid
//print the vaules and correct spacing of the grid
//for loop to make it elegant
for (int i = 0; i < 9; i++) //you know that there are only 9 spaces to print so don't overcomplicate this but you could use i < gridSize, I think...
{
printf (" %d ", *(ticTacToeGrid + i));
if (i = 8)
{
printf ("\n\n\n");
return 1;
...
int any_plays_left(void);
{
// looks at the board state (pass current grid) and checks to see if there are any plays that haven't been made (if value is still '\0' (default))
// print number of plays remaining
int moves = 0;
char *ptr = &ticTacToeGrid[0][0];
for (int i = 0; i < 9; i ++)
{
if (*(ptr + i) == '\0')
{
...
/*
char did_someone_win(*currentGrid)
{
//pass the current grid and check for win conditions
*ptr = currentGrid;

if ((*ptr + 0) == (*ptr + 1)) && ((*ptr + 1) == (*ptr + 2)) && ((*ptr + 0) != '\0'))
{
}
}
*/
...
char ticTacToeGrid[3][3]; //empty grid, this is the game board that they players will be using
char ticTacToeLegend[3][3] = { '1','2','3','4','5','6','7','8','9' };  //initialized second array for Legend Display

int main(void)
{
	printf("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
	printf("X  TTTTTTTTT   III      CCC            TTTTTTTTT      AAA         CCC            TTTTTTTTT      OOO      EEEEEEEEE  X\n");
	printf("X  TTTTTTTTT   III    CCC CCC          TTTTTTTTT     AAAAA      CCC CCC          TTTTTTTTT    OOO OOO    EEEEEEEEE  X\n");
	printf("X  TTTTTTTTT   III   CCC   CCC         TTTTTTTTT     AAAAA     CCC   CCC         TTTTTTTTT   OOO   OOO   EEE    EE  X\n");
	printf("X     TTT      III   CCC   CCC   ---      TTT       AAA AAA    CCC   CCC   ---      TTT      OOO   OOO   EEEEE      X\n");
	printf("X     TTT      III   CCC         ---      TTT       AAA AAA    CCC         ---      TTT      OOO   OOO   EEEEE      X\n");
	printf("X     TTT      III   CCC   CCC   ---      TTT       AAAAAAA    CCC   CCC   ---      TTT      OOO   OOO   EEEEE      X\n");
	printf("X     TTT      III   CCC   CCC            TTT      AAAAAAAAA   CCC   CCC            TTT      OOO   OOO   EEE    EE  X\n");
	printf("X     TTT      III    CCC CCC             TTT      AAA   AAA    CCC CCC             TTT       OOO OOO    EEEEEEEEE  X\n");
	printf("X     TTT      III      CCC               TTT      AAA   AAA      CCC               TTT         OOO      EEEEEEEEE  X\n");
	printf("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
	int menuSelection = 0; //
	int number = 0;
	...
	do //does this until something else happens
	{
		...
		menuSelection = atoi(tempSelect); //ascii to integer
		...
		case 1: //main function
			...
			{
				print_the_grid(&(ticTacToeLegend[0][0]));  //prints legend
